...
class FailureRunView implements ITestRunView, IMenuListener {
	...
	public FailureRunView(CTabFolder tabFolder, TestRunnerViewPart runner) {
		...
		failureTab.setToolTipText(CppUnitMessages.getString("FailureRunView.tab.tooltip")); //$NON-NLS-1$
	...
	public String getName() {
		return CppUnitMessages.getString("FailureRunView.tab.title"); //$NON-NLS-1$
	}
	...
	private String getClassName() {
		...
		className=HierarchyRunView.extractClassName(className);
		return(className);
	}
...
//				manager.add(new RerunAction(fRunnerViewPart, className, methodName));
	...
	private void updateTableItem(TestRunInfo testInfo, TableItem tableItem) {
		tableItem.setText(HierarchyRunView.filterFirstNumbers(testInfo.fTestName));
