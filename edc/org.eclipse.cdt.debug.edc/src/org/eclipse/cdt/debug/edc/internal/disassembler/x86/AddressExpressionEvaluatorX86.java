...
public class AddressExpressionEvaluatorX86 implements IAddressExpressionEvaluator {
	...
	public IAddress evaluate(ExecutionDMC context, String expression, Registers regService, Memory memService)
		...
		if (expression.startsWith("*")) {
			...
			if (tokenCnt == 1) {
				...
				if (token.startsWith("%")) {
					...
					String val = regService.getRegisterValue(context, regName);
				...
				if (baseReg != null)
					base = Long.valueOf(regService.getRegisterValue(context, baseReg), 16);
				if (indexReg != null)
					index = Long.valueOf(regService.getRegisterValue(context, indexReg), 16);
		...
		if (ret == null && memAddr != null) {
			...
			IStatus st = memService.getMemory(context, memAddr, memBuffer, 4, 1);
