 ...
 * @since 2.1
 */
public class EDCSuspendTrigger extends DsfSuspendTrigger {
    
    public EDCSuspendTrigger(DsfSession session, ILaunch launch) {
    ...
    protected void getLaunchTopContainers(final DataRequestMonitor<IContainerDMContext[]> rm) {
        try {
            getSession().getExecutor().execute(new DsfRunnable() {
                public void run() {
                    ...
                    if (processService == null) {
                        rm.setStatus(new Status(IStatus.ERROR, EDCDebugUI.PLUGIN_ID, IDsfStatusConstants.INVALID_STATE, "Not available", null)); //$NON-NLS-1$
                        rm.done();
                        return;
                    }
            ...
            rm.setStatus(new Status(IStatus.ERROR, EDCDebugUI.PLUGIN_ID, IDsfStatusConstants.INVALID_STATE, "Not available", e)); //$NON-NLS-1$
            rm.done();
        }
    }
}
