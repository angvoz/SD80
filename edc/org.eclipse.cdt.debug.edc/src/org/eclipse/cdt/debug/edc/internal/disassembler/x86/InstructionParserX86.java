...
import org.eclipse.cdt.debug.edc.internal.disassembler.x86.DisassemblerX86.IDisassemblerOptionsX86;
...
public class InstructionParserX86 {
	...
	public DisassembledInstruction getResult() throws CoreException {
		if (!parsed) {
	...
	private String parseOpcode(int opcodeByteCnt, byte opcodeLastByte) throws CoreException {
		...
		if (operandDescs != null) {
			...
			for (int i = 0; i < opCnt; i++) {
				...
				// operand
				{
					...
					//
					if (isMemoryOperand
							|| AssemblyFormatter.sInstructionsSuffixFromRegisterOperand.contains(opc.getName()))
						nameSizeSuffix = AssemblyFormatter.instructionNameSizeSuffix(operandSize);
				}
			}
		}
		...
		return AssemblyFormatter.formatInstruction(instPrefixes, opc.getName(), nameSizeSuffix, operandStrings);
	}
	...
	private OpcodeX86 lookupTwoByteOpcode(int opcodeID) throws CoreException {
		...
		if (opc == null)
			throw EDCDebugger.newCoreException(MessageFormat.format(
					"Two-byte opcode \"{0} {1}\" is not supported by the disassembler yet.", AssemblyFormatter
							.toHexString(opcode.get(0), false), AssemblyFormatter.toHexString(opcodeID, false)));
	...
	private OpcodeX86 lookupThreeByteOpcode(int opcodeID) throws CoreException {
		...
		if (opc == null)
			throw EDCDebugger.newCoreException(MessageFormat.format(
					"Three-byte opcode \"{0} {1}\" is not supported by the disassembler yet.", AssemblyFormatter
							.toHexString(opcode.get(0), false), AssemblyFormatter.toHexString(opcode.get(1), false),
					AssemblyFormatter.toHexString(opcodeID, false)));
	...
	private String parseOperand(String operandDescriptor) throws CoreException {
		...
		if (isRegisterID(operandDescriptor)) {
			...
			ret = AssemblyFormatter.formatRegister(reg.toLowerCase(), indirectAddressing);
		} else {
			// addressing operand
	...
	public String addressing_A(char[] operandType) throws CoreException {
		...
		case 'p':
			...
			operand = AssemblyFormatter.formatFarPointer(seg, offset);
			break;
	...
	public String addressing_I(char[] operandType) throws CoreException {
		...
		return AssemblyFormatter.formatImmediate(imm);
	}
	...
	public String addressing_J(char[] operandType) throws CoreException {
		...
		return AssemblyFormatter.formatForCode(jumpToAddr);
	}
	...
	public String addressing_O(char[] operandType) throws CoreException {
		...
		return AssemblyFormatter.formatOffset(offset);
	}
	...
	public String addressing_X(char[] operandType) throws CoreException {
		...
		return AssemblyFormatter.registerPair_DSrSI(addressMode);
	}
	...
	public String addressing_Y(char[] operandType) throws CoreException {
		...
		return AssemblyFormatter.registerPair_ESrDI(addressMode);
	}
	...
	private String memoryFromModRM(char addressingMethod, char[] operandType) throws CoreException {
		...
		return AssemblyFormatter.format(indirectAddressing, useDisplacement ? displacement : null, baseRegID,
	...
	private String getRegisterName(int regID, char addressingMethod, char[] operandType) throws CoreException {
		...
		try {
			...
			case 'E':
			case 'R':
				names = (AssemblyFormatter.sGPRNames.get(key));
				break;
			case 'C':
				names = AssemblyFormatter.sControlRegisterNames;
				break;
			case 'D':
				names = AssemblyFormatter.sDebugRegisterNames;
				break;
			case 'S':
				names = AssemblyFormatter.sSegmentRegisterNames;
				break;
	...
	private String getSegmentRegisterFromPrefix() {
		...
		for (int p : prefixes) {
			if (OpcodeX86.sPrefixesForOperand.contains(p)) {
				prefixesUsed.add(p);
				seg = AssemblyFormatter.formatPrefix(p);
			}
		}
