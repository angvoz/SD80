...
//		}
	...
	static class ClassesInSuitContentProvider implements IStructuredContentProvider {
		...
		{
			Vector v=new Vector();
			if(r instanceof ICContainer)
			{
				ICContainer p=(ICContainer)r;
				ICElement [] elements=p.getChildren();
				for(int i=0;i<elements.length;i++)
				{
					if(elements[i] instanceof ICContainer)
					{
			...
			ITranslationUnit [] array=new ITranslationUnit[v.size()];
			for(int i=0;i<v.size();i++)
			{
				array[i]=(ITranslationUnit)v.elementAt(i);
			}
			return array;
		}
		public void dispose() {
